// ✅ UPDATED index.js for Scrum Pointing App
// Includes: reconnection grace period, role/avatar-independent rejoin, connection status tracking,
// plus FULL device‐info emission on every participantsUpdate.
...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ nickname: name, room, role, avatar, emoji, device }) => {
    ...
    if (!rooms[room]) {
      rooms[room] = {
        ...
        currentStory: '',
        disconnectTimers: {},
        devices: {}          // ← track device types here
      };
    }
    ...
    r.devices[nickname] = device;              // ← store device on join

    const connectedNicknames = [...io.sockets.sockets.values()]
      .filter(s => s.rooms.has(room))
      .map(s => s.nickname);

    // Emit participantsUpdate *including* devices
  ...
  socket.on('forceRemoveUser', (targetNickname) => {
    ...
    delete room.devices[targetNickname];             // ← also delete device

    // Notify all, with corrected variable (`room.devices`)
    io.to(currentRoom).emit('participantsUpdate', {
      ...
      connected: [...io.sockets.sockets.values()]
                    ...
                    .map(s => s.nickname),
      devices:   room.devices                      // ← use room.devices, not “r” which is undefined here
    });
  ...
  socket.on('updateMood', ({ nickname: name, emoji }) => {
    if (rooms[currentRoom]) {
      ...
      io.to(currentRoom).emit('participantsUpdate', {
        ...
        devices:   rooms[currentRoom].devices  // ← keep sending devices on mood updates
      });
    }
  });
  ...
  socket.on('logout', () => {
    ...
    delete r.devices[nickname];                  // ← remove device on logout

    // Broadcast updated roster (now including devices and connected)
    ...
    io.to(currentRoom).emit('participantsUpdate', {
      ...
      devices:   r.devices                        // ← send the updated devices object
    });
  ...
  socket.on('disconnect', () => {
    ...
    io.to(currentRoom).emit('participantsUpdate', {
      ...
      moods:     r.moods,
      connected: connectedNow,
      devices:   r.devices                    // ← include devices here too
    });
